...
/-- Spectral flatness ↔ AB: the AB condition is a special case of flatness
    with c = 2^{n+1}, and flatness at any level implies AB. -/
theorem spectrFlat_iff_ab {n : ℕ} (f : F2Vec n → F2Vec n) :
  ...
  · intro ⟨c, _, hf⟩ a b hb
    ...
    · left; exact_mod_cast pow_eq_zero_iff (n := 2) (by omega) |>.mp h
    · right
      -- Without Parseval, we cannot determine c = 2^{n+1} from flatness alone.
      -- But AB requires W^2 = 2^{n+1}, so this direction needs Parseval.
      sorry
  · intro hf
    exact ⟨(2 : ℤ) ^ (n + 1), by positivity, fun a b hb =>
