...
/-!
# Foundational Layer: Automorphism Base

Semilinear operators, additive polynomial evaluation, and support theory
for the automorphism infrastructure of translation planes.

From "Permutation polynomials and translation planes of even order"
by U. Dempwolff and P. Müller (Adv. Geom. 2013), Section 4.
-/
...
/-- Additive polynomial L(x) = ∑_{i=0}^{n-1} coeffs(i) · x^{p^i}. -/
def additivePolyEval (n : ℕ) (coeffs : Fin n → F) (x : F) : F :=
  ∑ i : Fin n, coeffs i * x ^ (p ^ (i : ℕ))

/-- The support spi(L) = {i | aᵢ ≠ 0}. -/
...
-- ═══════════════════════════════════════════
-- Semilinear operator properties
-- ═══════════════════════════════════════════
...
-- ═══════════════════════════════════════════
-- Additive polynomial properties
-- ═══════════════════════════════════════════
