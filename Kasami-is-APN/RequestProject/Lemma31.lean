...
lemma PMap31_inj_iff_Delta_sub_bij (L : F →ₗ[K] F) {M : F → F}
    ...
    ∀ y₁ y₂ : F, y₁ ≠ y₂ →
      Function.Bijective (Delta L M y₁ - Delta L M y₂) := by
  constructor
  · intro hP y₁ y₂ hy₁₂
    have h_ker : ∀ x, (Delta L M y₁ - Delta L M y₂) x = 0 → x = 0 := by
      ...
      have := hP h_eq; aesop
    exact ⟨LinearMap.ker_eq_bot.mp (LinearMap.ker_eq_bot'.mpr h_ker),
           LinearMap.surjective_of_injective
             (LinearMap.ker_eq_bot.mp (LinearMap.ker_eq_bot'.mpr h_ker))⟩
  · intro h x y hxy
    by_contra hneq
...
lemma bijective_iff_adjoint_bijective (T : F →ₗ[K] K)
  ...
  · intro h
    have h_nondeg : ∀ v w : F, (∀ u : F, T (A u * v) = T (A u * w)) → v = w := by
      ...
      exact sub_eq_zero.mp (hT _ fun x => by
        obtain ⟨u, rfl⟩ := h.2 x; simpa [mul_comm] using h_eq u)
    have h_adj_inj : Function.Injective Aadj := by
      exact fun v w hvw => h_nondeg v w fun u => by rw [hAdj, hAdj, hvw]
    exact ⟨h_adj_inj, by exact LinearMap.surjective_of_injective h_adj_inj⟩
  · intro hAadj_bijective
    have hA_inj : Function.Injective A := by
      ...
      exact sub_eq_zero.mp (hT (u - v) fun w => by
        obtain ⟨w, rfl⟩ := hAadj_bijective.2 w
        simpa [sub_mul] using sub_eq_zero.mpr (h_eq w))
...
theorem lemma_3_1 (T : F →ₗ[K] K)
  ...
  have h_bij_iff : ∀ y₁ y₂ : F,
    ...
    apply bijective_iff_adjoint_bijective T hT
      (Delta L M y₁ - Delta L M y₂) (Delta Ladj Minv (M y₁) - Delta Ladj Minv (M y₂))
