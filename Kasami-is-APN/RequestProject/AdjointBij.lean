...
lemma adjoint_swap_bij {n : ℕ} (hn : Fintype.card F = p ^ n) (hn1 : 1 ≤ n)
  ...
  by_cases he : e = 0
  · simp [he] at hel
    have := Finset.card_eq_two.mp hel
    obtain ⟨x, y, hxy⟩ := this; simp_all +decide [Fintype.card_eq_one_iff]
    have hL₁_zero : L₁ 0 = 0 := by simpa using hL₁_add 0 0
    have hL₂_zero : L₂ 0 = 0 := by simpa using hL₂_add 0 0
    ...
    have hL₂_one : L₂ 1 = 1 := by
      ...
      · exact this.symm
      · exact absurd hn (ne_of_lt (lt_of_lt_of_le
          (show 2 < p ^ 2 by nlinarith only [hp.1.two_le])
          (Nat.pow_le_pow_right hp.1.pos
            (Nat.succ_le_succ (Nat.succ_le_succ n.zero_le)))))
    simp_all +decide [Finset.ext_iff, Set.ext_iff]
    cases hxy.2 0 <;> cases hxy.2 1 <;>
    ...
    · simp [hl] at hel; subst hl; simp only [pow_zero, mul_one]
      exact Finite.injective_iff_bijective.mp (fun a b hab => by
        ...
        exact (by
          ...
          have : p = 2 := by
            have := congr_arg Even hn
            norm_num [hp.1.even_iff, parity_simps] at this
            exact this.1
          generalize_proofs at *
          subst this
          rcases n with (_ | _ | n) <;> norm_num [Nat.pow_succ'] at *
          have := Finset.card_eq_two.mp hel
          obtain ⟨x, y, hxy⟩ := this
          simp_all +decide [Finset.ext_iff]
          cases hxy.2 0 <;> cases hxy.2 1 <;> cases hxy.2 a <;> cases hxy.2 b <;>
