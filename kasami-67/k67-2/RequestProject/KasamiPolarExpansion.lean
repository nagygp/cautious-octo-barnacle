...
This file implements `kasami_polar_expansion`, the trace-level identity connecting
the polar form of the power function to the associated linearized operator,
using the **Route 1 (Trace Adjoint)** approach from the project roadmap.

## Mathematical Background

For a finite field `F = GF(2^n)` of characteristic 2 and the power exponent
`d = 2^k + 1`, the polar form

    `polar(x, y) = (x + y)^d + x^d + y^d`

expands algebraically to `x · y^(2^k) + x^(2^k) · y`.

This is a **bilinear** form (GF(2)-linear in each argument), and the Route 1
Adjoint Logic proves:

    `Tr(a · polar(x, y)) = Tr(x · L_a(y))`

where `L_a(y) = a · y^(2^k) + a^(2^(n-k)) · y^(2^(n-k))` is the associated
linearized operator.

## Proof Structure

The proof uses `calc` blocks throughout to keep each rewriting step small
and avoid heartbeat timeouts:

1. **Polar expansion** — algebraic identity for `(x+y)^(2^k+1)`.
2. **Trace Frobenius invariance** — `Tr(w^(2^j)) = Tr(w)`.
3. **Trace adjoint** — `Tr(u · v^(2^j)) = Tr(u^(2^(n-j)) · v)`.
4. **Bridge** — connects polar form to `L_a`.
5. **Main theorem** — `kasami_polar_expansion`, the full calc chain.
6. **Radical = Kernel** — corollary via trace non-degeneracy.

## References

- Canteaut, Charpin, Dobbertin, "Binary m-sequences with three-valued
  crosscorrelation" (2000)
- Gold, "Maximal recursive sequences with 3-valued recursive
  cross-correlation functions" (1968)
-/
...
/-! ## Section 2: Frobenius Algebra -/

/-- **Trace Frobenius invariance.** `Tr(w^(2^j)) = Tr(w)` for all `j`.

The proof proceeds by induction on `j`, using the fact that squaring is a
GF(2)-algebra automorphism (Frobenius endomorphism) and the trace is
invariant under algebra automorphisms.

Each step of the `calc` block has low heartbeat cost. -/
...
/-! ## Section 3: Gold / Kasami Polar Expansion (Algebraic) -/
...
The `calc` block:
1. Rewrite `(x+y)^(d) = (x+y) · (x+y)^(2^k)`.
2. Apply Frobenius additivity: `(x+y)^(2^k) = x^(2^k) + y^(2^k)`.
3. Expand and cancel in characteristic 2. -/
...
/-! ## Section 4: Element identity `x^(2^n) = x` -/
...
/-! ## Section 5: Trace Adjoint Identity -/
...
This is the key identity for Route 1. The `calc` proof applies Frobenius
invariance `Tr(w) = Tr(w^(2^m))` with `m = n − j`:

    `Tr(u · v^(2^j)) = Tr((u · v^(2^j))^(2^(n−j)))     [Frobenius inv.]
                      = Tr(u^(2^(n−j)) · v^(2^j·2^(n−j))) [mul_pow]
                      = Tr(u^(2^(n−j)) · v^(2^n))          [exponent arith.]
                      = Tr(u^(2^(n−j)) · v)`               [v^(2^n) = v]  -/
...
/-! ## Section 6: Trace Non-degeneracy -/

/-- **Trace non-degeneracy.** If `Tr(x · z) = 0` for all `x`, then `z = 0`.

This follows from the non-degeneracy of the trace form for separable extensions:
if `z ≠ 0`, we can find `x = w/z` for a suitable `w` with `Tr(w) ≠ 0`. -/
...
/-! ## Section 7: The Linearized Operator (Route 1) -/

/-- The linearized operator `L_a(y)` for the Gold/Kasami exponent via Route 1:

    `L_a(y) = a · y^(2^k) + a^(2^(n−k)) · y^(2^(n−k))`

This is the adjoint of `x ↦ a · (x · y^(2^k) + x^(2^k) · y)` with respect
to the trace bilinear form `⟨u, v⟩ = Tr(u · v)`. -/
noncomputable def goldLinearizedOp (k : ℕ) (a y : F) : F :=
  a * y ^ (2 ^ k) + a ^ (2 ^ (Module.finrank (ZMod 2) F - k)) *
    y ^ (2 ^ (Module.finrank (ZMod 2) F - k))

/-! ## Section 8: The Bridge Lemma -/

/-- **Bridge Lemma.** The polar form under trace equals `Tr(x · L_a(y))`:

    `Tr(a · (x · y^(2^k) + x^(2^k) · y)) = Tr(x · L_a(y))`

This connects the algebraic polar expansion to the linearized operator
via the trace adjoint identity.

The `calc` proof:
1. Split by trace linearity into two terms.
2. The first term is already `Tr(x · (a · y^q))`.
3. Apply trace adjoint to the second term to get `Tr(x · (a^q̃ · y^q̃))`.
4. Recombine into `Tr(x · L_a(y))`. -/
lemma gold_bridge (k : ℕ) (a x y : F) (hk : k ≤ Module.finrank (ZMod 2) F)
  ...
  -- Abbreviate the complementary exponent for readability
  ...
  calc fieldTrace (a * (x * y ^ (2 ^ k))) +
      ...
      -- Step 1: Reorder to isolate x in first term, rewrite second
      = fieldTrace (x * (a * y ^ (2 ^ k))) +
        fieldTrace ((a * y) * x ^ (2 ^ k)) := by ring_nf
      -- Step 2: Apply trace adjoint to the second summand:
      -- Tr((a·y) · x^(2^k)) = Tr((a·y)^(2^(n-k)) · x)
    _ = fieldTrace (x * (a * y ^ (2 ^ k))) +
        fieldTrace ((a * y) ^ (2 ^ nk) * x) := by
          congr 1
          exact trace_adjoint (a * y) x k hk hcard
      -- Step 3: Expand (a·y)^(2^(n-k)) = a^(2^(n-k)) · y^(2^(n-k))
    _ = fieldTrace (x * (a * y ^ (2 ^ k))) +
        fieldTrace (x * (a ^ (2 ^ nk) * y ^ (2 ^ nk))) := by
          congr 1; rw [mul_pow]; ring_nf

/-! ## Section 9: Main Theorem — `kasami_polar_expansion` -/
...
For the exponent `d = 2^k + 1` over a finite field of characteristic 2:

    `Tr(a · ((x+y)^d + x^d + y^d)) = Tr(x · L_a(y))`

where `L_a(y) = a · y^(2^k) + a^(2^(n-k)) · y^(2^(n-k))`.

This is the central identity connecting the polar form to the linearized
operator.  The `calc` proof chains:

1. **`gold_polar_expand`** — rewrite the polar form algebraically.
2. **`gold_bridge`** — apply the trace adjoint to reach `L_a`. -/
theorem kasami_polar_expansion (k : ℕ) (a x y : F)
  ...
  calc fieldTrace
      ...
      -- Step 1: Algebraic polar expansion
      = fieldTrace (a * (x * y ^ (2 ^ k) + x ^ (2 ^ k) * y)) := by
          congr 1; rw [gold_polar_expand]
      -- Step 2: Bridge to the linearized operator
    _ = fieldTrace (x * goldLinearizedOp k a y) :=
          gold_bridge k a x y hk hcard

/-! ## Section 10: Radical = Kernel (Route 1 Corollary) -/

/-- The radical of `Q_a`: the set of `y` such that `B_a(x, y) = 0` for all `x`. -/
...
Follows from `kasami_polar_expansion`:
- **Forward:** If `y ∈ rad`, then `Tr(x · L_a(y)) = 0` for all `x`,
  so `L_a(y) = 0` by trace non-degeneracy.
- **Backward:** If `L_a(y) = 0`, then `Tr(x · L_a(y)) = Tr(0) = 0`,
  so `B_a(x, y) = 0` for all `x`. -/
theorem kasami_radical_eq_kernel (k : ℕ) (a : F)
  ...
  constructor
  · -- Forward: radical → kernel (via trace non-degeneracy)
    intro hy
    apply trace_nondeg
    intro x
    have := hy x
    ...
    exact this
  · -- Backward: kernel → radical (direct substitution)
    intro hy x
