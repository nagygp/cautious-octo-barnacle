...
Foundational definitions and lemmas for the automorphism theory of
translation planes from "Permutation polynomials and translation planes
of even order" by U. Dempwolff and P. Müller (Adv. Geom. 2013).

**Setting.** Let `F = GF(pⁿ)` be a finite field of characteristic `p`.
An additive polynomial `L(X) = ∑ aᵢ X^{pⁱ}` defines a GF(p)-linear
endomorphism of `F`. We define:

- The **semilinear operator** `T_r(a) : x ↦ a · x^{p^r}` for `a ∈ F`,
  `0 ≤ r < n`. These are the elements of the normalizer of the Singer
  group `C = {T_0(a) | a ∈ F*}` in `GL(V)` (Lemma 4.1).

- The **support** `spi(L) = {i | aᵢ ≠ 0}` of an additive polynomial.

- **Lemma 4.2:** The support transforms predictably under conjugation:
  `spi(T_s(a) · L · T_t(b)⁻¹) = {i + r | i ∈ spi(L)}` where `r = s − t`.

This layer provides the algebraic infrastructure used throughout
...
/-- `T_r(a)(0) = 0`. -/
lemma semilinearOp_zero (r : ℕ) (a : F) :
    semilinearOp p r a (0 : F) = 0 := by
  simp [semilinearOp, zero_pow (pp_ne_zero p r)]

/-- `T_0(1) = id`, i.e. `T_0(1)(x) = x`. -/
lemma semilinearOp_zero_one (x : F) :
    semilinearOp p 0 (1 : F) x = x := by
  simp [semilinearOp]

/-- `T_0(a)(x) = a · x` — the purely linear (scalar multiplication) case. -/
lemma semilinearOp_zero_eq_mul (a x : F) :
    semilinearOp p 0 a x = a * x := by
  simp [semilinearOp]

/-- `T_r(a)` is additive in characteristic `p`:
    `T_r(a)(x + y) = T_r(a)(x) + T_r(a)(y)`. -/
...
/-- Frobenius power injectivity: `x^{p^r} = y^{p^r} → x = y` in a reduced
    ring of characteristic `p`. -/
lemma frobenius_pow_injective (r : ℕ) {x y : F} (h : x ^ (p ^ r) = y ^ (p ^ r)) :
    x = y :=
...
lemma additivePolyEval_zero (n : ℕ) (coeffs : Fin n → F) :
  ...
  simp only [additivePolyEval, zero_pow (pp_ne_zero p _), mul_zero,
...
lemma additivePolyEval_smul (n : ℕ) (coeffs : Fin n → F) (c : F)
    (hc : c ^ p = c) (x : F) :
    additivePolyEval p n coeffs (c * x) =
    c * additivePolyEval p n coeffs x := by
...
/-- **Single-term conjugation.** Conjugating the monomial `aᵢ · x^{p^i}`
    by `T_s(a)` on the left and `T_t(b⁻¹)` on the right gives
    a monomial of degree `p^{t+i+s}`. -/
...
/-- **Lemma 4.2 — Coefficient nonvanishing.**
    If `aᵢ ≠ 0` and `a, b⁻¹ ≠ 0`, then the coefficient of the
    conjugated monomial is nonzero. -/
lemma conjugation_coeff_ne_zero {a ai b_inv : F} (ha : a ≠ 0) (hai : ai ≠ 0)
    (hb : b_inv ≠ 0) (i s : ℕ) :
    a * (ai * b_inv ^ (p ^ i)) ^ (p ^ s) ≠ 0 :=
  mul_ne_zero ha (pow_ne_zero _ (mul_ne_zero hai (pow_ne_zero _ hb)))

/-- **Lemma 4.2 — Coefficient vanishing characterization.**
    The conjugated coefficient is zero iff `aᵢ = 0` (when `a, b⁻¹ ≠ 0`). -/
lemma conjugation_coeff_eq_zero_iff {a ai b_inv : F} (ha : a ≠ 0)
    (hb : b_inv ≠ 0) (i s : ℕ) :
...
/-- **Lemma 4.2 (evaluation form).**
    The composed map `x ↦ T_s(a)(L(T_t(b⁻¹)(x)))` evaluates to a sum
    whose `i`-th term has exponent `p^{t+i+s}` and coefficient
    `a · (aᵢ · (b⁻¹)^{p^i})^{p^s}`. -/
lemma lemma_4_2_eval (n : ℕ) (coeffs : Fin n → F)
    (a b_inv : F) (s t : ℕ) (x : F) :
...
/-- **Lemma 4.2 (support characterization).**
    The conjugated polynomial has the same "nonzero pattern" as the original:
    the `i`-th conjugated coefficient is nonzero iff `coeffs i ≠ 0`.
    This captures the paper's statement that conjugation shifts the support
    indices by `s − t` without adding or removing elements. -/
theorem lemma_4_2_support (n : ℕ) (coeffs : Fin n → F)
    {a b_inv : F} (ha : a ≠ 0) (hb : b_inv ≠ 0) (s : ℕ) (i : Fin n) :
