...
/- **FALSE as stated**: Missing hypothesis gcd(k,n) = 1.
   Counterexample: GF(4), k=2: d = 13 ≡ 1 (mod 3), so G(x) = x.
   Then D_1 G(y) = 1 for all y, but L_2(α) = α ≠ 0 for α ∈ GF(4)\GF(2).
   The correct version with gcd(k,n)=1 and 3∤n is
   `kasamiDiff_eq_implies_two_solutions` below. -/
/- Commented out: FALSE without field-size hypotheses.
...
/-- **Corrected version**: Equal Kasami first-derivatives ⟹ y₁ = y₂ or y₁ = y₂ + 1,
    when gcd(k,n) = 1, 3 ∤ n, and n is odd.

    **Proof**: If z = y₁+y₂ ∉ {0,1}, the CCD norm equation
    `D^{2^k}c + Dc^{2^k} = z^{2^{3k}} + z` holds (by `ccd_two_solution_eq'`).
    Then `z^{2^{3k}} + z = M_k(L_k(z))`. Since ker(L_k) = {0} (from
    `linPolyL_ker_trivial_of_three_ndvd`), we have L_k(z) = 0 iff z = 0.
    L_k(z) ∈ ker(M_k) = GF(2) would force z^{2^{3k}}+z = 0 hence z ∈ {0,1}.
    The remaining case L_k(z) ∉ GF(2) requires the deep CCD kernel argument. -/
theorem kasamiDiff_eq_implies_two_solutions (n : ℕ) (hn : 0 < n) (k : ℕ) (hk : 0 < k)
...
theorem kasamiDelta_two_to_one (n : ℕ) (hn : 0 < n) (k : ℕ) (hk : 0 < k)
  ...
  have h_diff : kasamiDiff k 1 b₁ = kasamiDiff k 1 b₂ := by
    unfold kasamiDelta kasamiDiff at *;
    grind
  rcases kasamiDiff_eq_implies_two_solutions n hn k hk hcard hgcd h3 hn_odd b₁ b₂ h_diff with h | h
  · exact Or.inl h.symm
  · right; rw [h]; ring_nf; rw [show (2 : F) = 0 from CharP.cast_eq_zero F 2]; ring
...
theorem kasami_apn (n : ℕ) (hn : 0 < n) (k : ℕ) (hk : 0 < k)
  ...
  have h_solutions : ∀ y₁ y₂ : F, kasamiDiff k 1 y₁ = c / a ^ (kasamiExp k) → kasamiDiff k 1 y₂ = c / a ^ (kasamiExp k) → y₁ = y₂ ∨ y₁ = y₂ + 1 := by
    intro y₁ y₂ hy₁ hy₂
    have h_eq : kasamiDiff k 1 y₁ = kasamiDiff k 1 y₂ := by rw [hy₁, hy₂]
    exact kasamiDiff_eq_implies_two_solutions n hn k hk hcard hgcd h3 hn_odd y₁ y₂ h_eq
