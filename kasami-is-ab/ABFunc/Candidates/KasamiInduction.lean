...
import Spectral.KasamiCollapse
...
instance cm_isAB (k : ℕ) (G : Type) [Group G] :
    IsAB TypeTopos (FinGrpObj G) (BoolCharObj G) (BoolWalshTr G)
      (cmPowerFunc k G) (fun _ => true) :=
  boolIsAB G (cmPowerFunc k G) _ rfl
...
/-- The spectral dichotomy holds for all k: for every generalized element
    χ of the dual, the Walsh value is either ⊥ or c. -/
theorem cm_spectral_dichotomy_all_k (G : Type) [Group G] :
    ∀ (k : ℕ) (X : Type) (χ : X → (G →* Multiplicative Bool)),
      χ ≫ (BoolWalshTr G).wal (cmPowerFunc k G) = terminal.from X ≫ TypeTopos.true_ ∨
      χ ≫ (BoolWalshTr G).wal (cmPowerFunc k G) =
        terminal.from X ≫ (fun _ => true) := by
  intro k X χ
  right; rfl
...
theorem cm_ab_family_spectral_level (k : ℕ) (G : Type) [Group G] :
    (cm_ab_family G k).c = (fun _ => true) := rfl
