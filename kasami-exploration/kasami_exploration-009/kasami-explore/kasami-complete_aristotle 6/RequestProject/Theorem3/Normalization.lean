...
/-- Normalized linearized operator: `Lnorm(y) = y^(2^k) + y + 1`.
    After dividing by `u^(2^k+1)` and substituting `y = x·u⁻¹`,
    the derivative equation becomes `Lnorm(y) = 0`. -/
def Lnorm (y : F) : F :=
  y ^ (2 ^ k) + y + 1
...
omit [Fintype F] [DecidableEq F] in
/-- The kernel of `Δ_u f` is in bijection with the roots of `Lnorm`. -/
