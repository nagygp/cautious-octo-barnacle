import CodeTheoryCryptoEquiv.Walsh.WalshAB
import CodeTheoryCryptoEquiv.Core.KasamiEvenK
import CodeTheoryCryptoEquiv.Core.CrossFormAnalysis
import CodeTheoryCryptoEquiv.Core.KasamiWalshDiv
...
lives in `CodeTheoryCryptoEquiv/DiffUniformity/KasamiDiffUniformity.lean`
...
/-! ## Layer K5: Literature-faithful AB statement (output-mask convention)

The predicate `WalshAB.IsAB` constrains the Walsh squares `walsh f a b ^ 2` for
every **input** mask `a ≠ 0` and every output mask `b`.  The standard textbook
almost-bent condition instead constrains every **output** mask `b ≠ 0` and every
input mask `a` (including `a = 0`): the component functions `x ↦ Tr (b · f x)`,
`b ≠ 0`, all have extended Walsh values in `{0, ±2^{(n+1)/2}}`.

For a *permutation* `f` the two conventions coincide on the relevant masks: the
only pairs they treat differently are `(a ≠ 0, b = 0)` (where `walsh f a 0 = 0`)
and `(a = 0, b ≠ 0)` (where, for a permutation, `walsh f 0 b = 0` as well).  The
Kasami map is a permutation under the standing hypotheses, so its AB property
holds in the literature-faithful output-mask form below.
-/

/-- For a permutation `f` over a finite field of characteristic two, the Walsh
coefficient at input mask `0` and any nonzero output mask `b` vanishes:
`walsh f 0 b = ∑ₓ χ (b · f x) = ∑_y χ (b · y) = 0`. -/
theorem walsh_zero_left_of_bijective {f : F → F} (hf : Function.Bijective f)
    {b : F} (hb : b ≠ 0) :
    walsh f 0 b = 0 := by
  have h_sum_zero : ∑ x : F, (WalshAB.χ (b * f x)) = 0 := by
    convert WalshAB.χ_sum_eq b using 1;
    · exact Equiv.sum_comp ( Equiv.ofBijective f hf ) fun x => χ ( b * x );
    · rw [ if_neg hb ];
  unfold walsh; aesop;
...
`walsh (x ↦ x ^ d k) a b ^ 2` lies in `{0, 2^{n+1}}`.  This is the standard
almost-bent condition on the component functions `Tr (b · x^{d k})`, `b ≠ 0`. -/
theorem kasami_is_ab_outputMask {n : ℕ} (hcard : Fintype.card F = 2 ^ n)
    (k : ℕ) (hk : k ≥ 1) (hkn : k < n) (hcop : Nat.Coprime k n) (hnodd : Odd n)
    (hn : n ≥ 1) :
    ∀ b : F, b ≠ 0 → ∀ a : F,
      walsh (fun x : F => x ^ d k) a b ^ 2 = 0 ∨
        walsh (fun x : F => x ^ d k) a b ^ 2 = (2 ^ (n + 1) : ℤ) := by
  intro b hb a
  by_cases ha : a = 0
  · subst ha
    left
    rw [walsh_zero_left_of_bijective
      (kasami_bijective hcard k hk hcop hnodd hn) hb]
    ring
  · exact kasami_is_ab hcard k hk hkn hcop hnodd hn a ha b
