...
import RequestProject.DiffUniformity.CCZEquiv
...
`RequestProject/DiffUniformity/Flystel.lean` and the CCZ-equivalence relation
`APN.CCZEquiv` of `RequestProject/DiffUniformity/CCZEquiv.lean`.
