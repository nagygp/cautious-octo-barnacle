...
`RequestProject/CodingTheory/LinearCode.lean` into a **Mathlib-PR-ready** form,
...
### Separate candidate (finite-field / crypto track)

* `FiniteFieldPowerMap.lean` — the **power-permutation characterisation** of a
  finite field ([Lidl–Niederreiter, *Finite Fields*, Thm 7.8]): for `d ≥ 1`,
  `x ↦ x^d` permutes `K` iff `Nat.Coprime d (#K − 1)`
  (`FiniteField.pow_bijective_iff`), together with the unit-group step
  (`units_pow_bijective_iff`), the forgetful bridge across the fixed point `0`
  (`pow_bijective_iff_units`), and the `Equiv` packaging (`FiniteField.powEquiv`).
  Mathlib already has the *group* version (`Nat.Coprime.pow_left_bijective`,
  `powCoprime`); this file supplies the *field* version (which includes `0`) as a
  small, general, dependency-light pearl. Mathlib-only imports; candidate home
  `Mathlib/FieldTheory/Finite/Basic.lean` (or a new
  `Mathlib/FieldTheory/Finite/PowerMap.lean`). It underpins the ZK/crypto S-box
  bijectivity certificates (Poseidon/MiMC/Rescue/Griffin `x ↦ x^d`).
