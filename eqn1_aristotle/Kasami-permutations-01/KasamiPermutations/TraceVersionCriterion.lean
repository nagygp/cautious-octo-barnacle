...
The library file `Theorem8C1.lean` proves that the *trace version*
`g(x) = q^{(Tr x)}(x) = (S(x) + Tr(x))·x^{(2ⁿ−1)−(2^k+1)}` of the Kasami
polynomial is a **permutation** of `𝔽_{2ⁿ}` in the single parity case
`k'` even, `n` odd (the single parity case treated in the original library).
...
proofs mirror `Theorem8C1.lean` verbatim except that the final parity computation
uses `S(1) + Tr(1) = (k' + n : F)`, which is `1` exactly when `k' + n` is odd.
-/

namespace Kasami.TraceVersionCriterion

open Finset BigOperators Kasami.FiniteField
open Kasami.TraceFreeCriterion

variable {F : Type*} [Field F] [Fintype F] [CharP F 2]

/-- The **trace version** `g(x) = q^{(Tr x)}(x)` of the Kasami map: it uses the
actual trace `Tr(x)` in place of a constant bit `ε`.  This is the `α = 1` object
`q₁` of the criterion. -/
...
criterion `Odd (kk + n)`, `S(x) + Tr(x) ≠ 0` for all `x ≠ 0`.
-/
theorem sTrace_add_trace_ne_zero_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have h_x_one : x ^ 2 + x = 0 := by
    have h_x_one : (pTrace k kk x) ^ (2 ^ k) + pTrace k kk x = x ^ 2 + x := by
      apply Kasami.TraceFreeCriterion.pTrace_telescope hn hkk' x;
    grind +ring;
  -- Since $x \neq 0$, we have $x = 1$.
  have h_x_one : x = 1 := by
    grind +suggestions;
  ...
  have h_sum : sTrace k kk 1 + truncTrace n 1 = (kk : F) + (n : F) := by
    simp +decide [ sTrace_one, Kasami.FiniteField.truncTrace ];
  obtain ⟨ m, hm ⟩ := hcrit; simp_all +decide [ Nat.even_add ] ;
  norm_cast at *; simp_all +decide [ Nat.even_add ] ;

/-
**A solution has `Q ≠ 0` (trace version, parity-general).**
the trace-version step-vanishing lemma.
-/
theorem sol_qPoly_ne_zero_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
    ...
    Kasami.TraceFreeCriterion.qPoly γ c k t ≠ 0 := by
  intro hq0
  have ht_step : t ^ (2 ^ k) = (γ * t) ^ (2 ^ k) + γ * t + 1 := by
    convert Kasami.TraceFreeCriterion.qPoly_zero_step hγ hcdef hq0 using 1;
  have ht_trace : truncTrace n t = (n : F) := by
    ...
    have ht_trace_one : truncTrace n 1 = (n : F) := by
      simp +decide [ Kasami.FiniteField.truncTrace ];
    grind;
  have ht_sTrace : sTrace k kk t = (γ * t) ^ 2 + γ * t + (kk : F) := by
    convert Kasami.TraceFreeCriterion.sTrace_telescope_gen hn hkk' ( Or.inr rfl ) _ using 1;
    rotate_left;
...
/-- **Root count, Case 2 (trace version, parity-general).**
the trace-version root-count image lemma. -/
theorem root_count_image_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have hQx : Kasami.TraceFreeCriterion.qPoly γ c k x ≠ 0 := by
    apply sol_qPoly_ne_zero_trace_gen hn hkk' hcrit;
    · grind +qlia;
    · exact hcdef;
    · grind +ring
  have hQy : Kasami.TraceFreeCriterion.qPoly γ c k y ≠ 0 := by
    apply sol_qPoly_ne_zero_trace_gen hn hkk' hcrit;
    · grind;
    · exact hcdef;
    · grind +ring
  have hQeq : Kasami.TraceFreeCriterion.qPoly γ c k x = Kasami.TraceFreeCriterion.qPoly γ c k y := by
    have hQeq : (Kasami.TraceFreeCriterion.qPoly γ c k x) ^ (2 ^ k - 1) = γ ^ (2 ^ k - 1) + γ⁻¹ ∧ (Kasami.TraceFreeCriterion.qPoly γ c k y) ^ (2 ^ k - 1) = γ ^ (2 ^ k - 1) + γ⁻¹ := by
      have hQeq : c ^ (2 ^ k) * x ^ (2 ^ (2 * k)) + x ^ (2 ^ k) + c * x + 1 = 0 ∧ c ^ (2 ^ k) * y ^ (2 ^ (2 * k)) + y ^ (2 ^ k) + c * y + 1 = 0 := by
        grind +suggestions;
      have hQeq : (Kasami.TraceFreeCriterion.qPoly γ c k x) ^ (2 ^ k) + (γ ^ (2 ^ k - 1) + γ⁻¹) * (Kasami.TraceFreeCriterion.qPoly γ c k x) = 0 ∧ (Kasami.TraceFreeCriterion.qPoly γ c k y) ^ (2 ^ k) + (γ ^ (2 ^ k - 1) + γ⁻¹) * (Kasami.TraceFreeCriterion.qPoly γ c k y) = 0 := by
        convert hQeq using 1;
        · rw [ ← Q_factor ];
          · grind +qlia;
          · exact hcdef;
        · grind +suggestions;
      have hQeq : (Kasami.TraceFreeCriterion.qPoly γ c k x) ^ (2 ^ k - 1) * (Kasami.TraceFreeCriterion.qPoly γ c k x) = (γ ^ (2 ^ k - 1) + γ⁻¹) * (Kasami.TraceFreeCriterion.qPoly γ c k x) ∧ (Kasami.TraceFreeCriterion.qPoly γ c k y) ^ (2 ^ k - 1) * (Kasami.TraceFreeCriterion.qPoly γ c k y) = (γ ^ (2 ^ k - 1) + γ⁻¹) * (Kasami.TraceFreeCriterion.qPoly γ c k y) := by
    ...
    have := Kasami.TraceFreeCriterion.pow2k1_inj hn hk hcop;
  ...
  · set d := x + y
    ...
    have hker : c * d ^ (2 ^ k) + γ ^ 2 * d = 0 := by
      simp_all +decide [ Kasami.TraceFreeCriterion.qPoly ];
    ...
    have htrd : truncTrace n d = 0 := by
      have := Kasami.FiniteField.trace_artin_schreier_zero hn k d; simp_all +decide [ add_comm, add_left_comm ] ;
      grind +suggestions
    have htrxy : truncTrace n x + truncTrace n y = 0 := by
      rw [ ← htrd, Kasami.FiniteField.truncTrace_add ]
    have hSd := Kasami.TraceFreeCriterion.sTrace_telescope_gen hn hkk' (Or.inl rfl) hstepd
    have hSadd := Kasami.TraceFreeCriterion.sTrace_add k kk x y
    simp_all +decide [ add_eq_zero_iff_eq_neg ];
    simp_all +decide [ Kasami.TraceFreeCriterion.qPoly, pow_add ];
    grind +ring

/-- **The root count (trace version, parity-general).**
the trace-version root count. -/
theorem root_count_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have h_trace_sq : ∃ γ : F, c = γ ^ (2 ^ k + 1) + γ := by
    apply Kasami.TraceFreeCriterion.ell0_root_imp_image hn hk hkn (by
...
theorem gmap_not_bijective_of_not_crit {n k kk : ℕ}
    ...
    (hexp : 2 ^ k + 1 < 2 ^ n - 1) :
    ¬ Function.Bijective (gmap (F := F) n k kk) := by
  intro hbij; have := hbij.injective; simp_all +decide ;
  convert @this 0 1 ?_ using 1;
  · simp +decide;
  · unfold gmap; simp +decide [ Kasami.TraceFreeCriterion.qeps, Kasami.FiniteField.truncTrace ] ;
...
theorem gmap_bijective_iff {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
    (hk : 0 < k) (hkn : k < n) (hcop : Nat.Coprime k n) (hkk' : k * kk % n = 1)
    (hexp : 2 ^ k + 1 < 2 ^ n - 1) :
    Function.Bijective (gmap (F := F) n k kk) ↔ Odd (kk + n) := by
  constructor
  · intro hbij
    by_contra hcrit
...
end Kasami.TraceVersionCriterion