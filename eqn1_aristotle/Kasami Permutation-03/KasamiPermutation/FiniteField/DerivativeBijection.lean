...
lemma P_inj_imp_DeltaBare_sub_bij' (L : F → F)
  ...
  have h_inj : Function.Injective (fun x => DeltaBare' L M y₁ x - DeltaBare' L M y₂ x) := by
    intro a b hab
    have hadd := DeltaBare_sub_additive' L hL_add M y₁ y₂ (b - a) a
    simp_all +decide [ sub_eq_iff_eq_add ]
    have := DeltaBare_sub_zero_imp_zero' L M hM_mul hM_inj hP_inj hy hadd
    simp_all +decide [ sub_eq_iff_eq_add ]
  exact Finite.injective_iff_bijective.mp h_inj

lemma DeltaBare_sub_bij_imp_P_inj' (L : F → F)
    (_hL_add : ∀ a b, L (a + b) = L a + L b) (hL0 : L 0 = 0)
    (M : F → F) (_hM_mul : ∀ a b, M (a * b) = M a * M b)
    (_hM_inj : Function.Injective M)
    (hDelta : ∀ y₁ y₂ : F, y₁ ≠ y₂ →
...
lemma DeltaBare_trace_adjoint' {n : ℕ} (_hn : Fintype.card F = p ^ n)
    (L₁ L₂ : F → F)
    (hAdj : ∀ w z, frobSum p n (L₁ w * z) = frobSum p n (w * L₂ z))
    (M Minv : F → F) (_hM_mul : ∀ a b, M (a * b) = M a * M b)
...
lemma additive_bij_iff_adj_bij' {n : ℕ} (_hn : Fintype.card F = p ^ n) (_hn1 : 1 ≤ n)
    (A Aadj : F → F)
    (_hA_add : ∀ a b, A (a + b) = A a + A b)
  ...
  · intro hA_bijective
    ...
    have hAadj_injective : Function.Injective Aadj := by
      ...
      have h_contra : ∀ z : F, frobSum p n (A z * (x - y)) = 0 := by
        have := hAadj_add ( x - y ) y; simp_all +decide ;
  ...
  · intro hAadj_bijective
    have hA_injective : Function.Injective A := by
      ...
      have h_diff_zero : ∀ z : F, frobSum p n ((x - y) * Aadj z) = 0 := by
        ...
        have := frobSum_add p n ( x * Aadj z - y * Aadj z ) ( y * Aadj z ) ; simp_all +decide ;
...
lemma DeltaBare_sub_bij_iff_adj' {n : ℕ} (hn : Fintype.card F = p ^ n) (hn1 : 1 ≤ n)
  ...
  any_goals tauto;
  · intro a b; simp +decide [ DeltaBare' ] ; ring;
...
theorem adjoint_swap_bij_bare {n : ℕ} (hn : Fintype.card F = p ^ n) (hn1 : 1 ≤ n)
  ...
  have hMinv_left : ∀ x : F, x ≠ 0 → (x ^ e) ^ l = x := by
    ...
    have h_exp : x ^ (e * l) = x := by
      rw [ ← Nat.mod_add_div ( e * l ) ( Fintype.card F - 1 ), hel ] ; simp +decide [ pow_add, pow_mul, hx_nonzero, pow_card_sub_one_eq_one' ] ; ring;
