...
import RequestProject.KasamiPermutation.FiniteField.AdjointBij
...
lemma truncTrace_ker_trivial {F : Type*} [Field F] [Fintype F] [CharP F 2]
    ...
    (hm_odd : Odd m) (_hm_pos : 1 < m) (_hm_lt : m < n)
...
lemma dicksonF_recursion_mul {F : Type*} [Field F] [CharP F 2] (m : ℕ) (x : F) :
  ...
  simp +decide [ ← pow_succ', add_comm 1, add_assoc ];
  ...
  · rw [ show 2 ^ ( m + 1 ) + 1 - 2 ^ ( i + 2 ) = 2 * ( 2 ^ m + 1 - 2 ^ ( i + 1 ) ) - 1 from ?_ ];
    ...
    · grind;
  · induction' ( range m ) using Finset.induction <;> simp_all +decide [ pow_succ', pow_mul' ];
    grind
...
lemma dicksonF_functional {F : Type*} [Field F] [CharP F 2]
  ...
  · refine' mul_left_cancel₀ h _;
    ...
    simp +decide [ show 2 ^ m * 2 = ( 2 ^ m - 1 ) * 2 + 2 by zify ; norm_num ; ring, pow_add, pow_mul' ] ; ring;
...
lemma LxXk_injective_on_units {F : Type*} [Field F] [Fintype F] [CharP F 2]
  ...
  have h_dickson : dicksonF m x⁻¹ = dicksonF m y⁻¹ := by
    ...
    · convert truncTrace_sq_mul_inv_eq_dicksonF m ( inv_ne_zero hx ) |> Eq.symm using 1 ; simp +decide [ pow_add ] ; ring;
      group;
...
theorem linearizedTimesPow_bijective {F : Type*} [Field F] [Fintype F] [CharP F 2]
    {n : ℕ} (hn : Fintype.card F = 2 ^ n) (m : ℕ)
    (hm_pos : 1 < m) (hm_odd : Odd m) (hm_lt : m < n)
    (hcop : Nat.Coprime m n) :
    Function.Bijective (fun x : F =>
