...
open FiniteFieldCharTwo Finset BigOperators
...
-/
/-- **L is Frobenius-equivariant** (the char-2 `truncTrace` face of
`Patterns.L.linTrace_frobenius`): `L_k(x²) = L_k(x)²`.  Each summand
`(x²)^{2^i} = (x^{2^i})²`, and squaring distributes over the sum in
characteristic 2. -/
lemma truncTrace_frobenius_sq {F : Type*} [CommSemiring F] [CharP F 2]
    (k : ℕ) (x : F) : truncTrace k (x ^ 2) = truncTrace k x ^ 2 := by
  simp only [truncTrace, CharTwo.sum_sq]
  congr 1; ext i; rw [← pow_mul, ← pow_mul, Nat.mul_comm]

/-- **The L↔F telescoping glue (Artin–Schreier), char-2 form.**  The linearized
trace sends the Artin–Schreier element `x² + x` to the Frobenius difference
`x^{2^k} + x`.  Structurally: `L` is additive (`truncTrace_add`) and
Frobenius-equivariant (`truncTrace_frobenius_sq`), so
`L_k(x²+x) = L_k(x)² + L_k(x)`, which telescopes to `x^{2^k}+x`
(`truncTrace_sq_add_self`).  See `Patterns.L.linTrace_artin_schreier` for the
characteristic-free version. -/
lemma truncTrace_artin_schreier {F : Type*} [CommRing F] [CharP F 2]
    (k : ℕ) (x : F) :
    truncTrace k (x ^ 2 + x) = x ^ (2 ^ k) + x := by
  rw [truncTrace_add, truncTrace_frobenius_sq, truncTrace_sq_add_self]
