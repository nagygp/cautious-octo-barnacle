...
`k'` even, `n` odd (`Dobbertin.Thm8C1.gmap_bijective`).
...
namespace Dobbertin.Thm8C1Gen

open Finset BigOperators DempwolffMueller
open Dobbertin Dobbertin.Thm8C1 Dobbertin.Thm5
...
`Dobbertin.Thm8C1.sTrace_add_trace_ne_zero`.
-/
theorem sTrace_add_trace_ne_zero_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have h_x_one : x ^ 2 + x = 0 := by
    have h_x_one : (pTrace k kk x) ^ (2 ^ k) + pTrace k kk x = x ^ 2 + x := by
      apply Dobbertin.Thm5.pTrace_telescope hn hkk' x;
    grind +ring;
  -- Since $x \neq 0$, we have $x = 1$.
  have h_x_one : x = 1 := by
    grind +suggestions;
  ...
  have h_sum : sTrace k kk 1 + truncTrace n 1 = (kk : F) + (n : F) := by
    simp +decide [ sTrace_one, DempwolffMueller.truncTrace ];
...
`Dobbertin.Thm8C1.sol_qPoly_ne_zero_trace`.
-/
theorem sol_qPoly_ne_zero_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
    ...
    Dobbertin.Thm5.qPoly γ c k t ≠ 0 := by
  intro hq0
  have ht_step : t ^ (2 ^ k) = (γ * t) ^ (2 ^ k) + γ * t + 1 := by
    convert Dobbertin.Thm5.qPoly_zero_step hγ hcdef hq0 using 1;
  have ht_trace : truncTrace n t = (n : F) := by
    ...
    have ht_trace_one : truncTrace n 1 = (n : F) := by
      simp +decide [ DempwolffMueller.truncTrace ];
    grind;
  have ht_sTrace : sTrace k kk t = (γ * t) ^ 2 + γ * t + (kk : F) := by
    convert Dobbertin.Thm5.sTrace_telescope_gen hn hkk' ( Or.inr rfl ) _ using 1;
    rotate_left;
...
`Dobbertin.Thm8C1.root_count_image_trace`. -/
theorem root_count_image_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have hQx : Dobbertin.Thm5.qPoly γ c k x ≠ 0 := by
    apply sol_qPoly_ne_zero_trace_gen hn hkk' hcrit;
    · grind +qlia;
    · exact hcdef;
    · grind +ring
  have hQy : Dobbertin.Thm5.qPoly γ c k y ≠ 0 := by
    apply sol_qPoly_ne_zero_trace_gen hn hkk' hcrit;
    · grind;
    · exact hcdef;
    · grind +ring
  have hQeq : Dobbertin.Thm5.qPoly γ c k x = Dobbertin.Thm5.qPoly γ c k y := by
    have hQeq : (Dobbertin.Thm5.qPoly γ c k x) ^ (2 ^ k - 1) = γ ^ (2 ^ k - 1) + γ⁻¹ ∧ (Dobbertin.Thm5.qPoly γ c k y) ^ (2 ^ k - 1) = γ ^ (2 ^ k - 1) + γ⁻¹ := by
      have hQeq : c ^ (2 ^ k) * x ^ (2 ^ (2 * k)) + x ^ (2 ^ k) + c * x + 1 = 0 ∧ c ^ (2 ^ k) * y ^ (2 ^ (2 * k)) + y ^ (2 ^ k) + c * y + 1 = 0 := by
        grind +suggestions;
      have hQeq : (Dobbertin.Thm5.qPoly γ c k x) ^ (2 ^ k) + (γ ^ (2 ^ k - 1) + γ⁻¹) * (Dobbertin.Thm5.qPoly γ c k x) = 0 ∧ (Dobbertin.Thm5.qPoly γ c k y) ^ (2 ^ k) + (γ ^ (2 ^ k - 1) + γ⁻¹) * (Dobbertin.Thm5.qPoly γ c k y) = 0 := by
        convert hQeq using 1;
        · rw [ ← Q_factor ];
          · grind +qlia;
          · exact hcdef;
        · grind +suggestions;
      have hQeq : (Dobbertin.Thm5.qPoly γ c k x) ^ (2 ^ k - 1) * (Dobbertin.Thm5.qPoly γ c k x) = (γ ^ (2 ^ k - 1) + γ⁻¹) * (Dobbertin.Thm5.qPoly γ c k x) ∧ (Dobbertin.Thm5.qPoly γ c k y) ^ (2 ^ k - 1) * (Dobbertin.Thm5.qPoly γ c k y) = (γ ^ (2 ^ k - 1) + γ⁻¹) * (Dobbertin.Thm5.qPoly γ c k y) := by
    ...
    have := Dobbertin.Thm5.pow2k1_inj hn hk hcop;
  ...
  · set d := x + y
    ...
    have hker : c * d ^ (2 ^ k) + γ ^ 2 * d = 0 := by
      simp_all +decide [ Dobbertin.Thm5.qPoly ];
    ...
    have htrd : truncTrace n d = 0 := by
      have := Dobbertin.Thm8.trace_artin_schreier_zero hn k d; simp_all +decide [ add_comm, add_left_comm ] ;
      grind +suggestions
    have htrxy : truncTrace n x + truncTrace n y = 0 := by
      rw [ ← htrd, DempwolffMueller.truncTrace_add ]
    have hSd := Dobbertin.Thm5.sTrace_telescope_gen hn hkk' (Or.inl rfl) hstepd
    have hSadd := Dobbertin.Thm5.sTrace_add k kk x y
    simp_all +decide [ add_eq_zero_iff_eq_neg ];
    simp_all +decide [ Dobbertin.Thm5.qPoly, pow_add ];
    grind +ring

/-- **The root count (trace version, parity-general).**  Generalizes
`Dobbertin.Thm8C1.root_count_trace`. -/
theorem root_count_trace_gen {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
  ...
  have h_trace_sq : ∃ γ : F, c = γ ^ (2 ^ k + 1) + γ := by
    apply Dobbertin.Thm5.ell0_root_imp_image hn hk hkn (by
...
theorem gmap_bijective_of_crit {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
    ...
    Function.Bijective (Thm8C1.gmap (F := F) n k kk) := by
  refine' ⟨ _, _ ⟩;
  · intro x y hxy
    by_cases hx : x = 0
    by_cases hy : y = 0
...
theorem gmap_not_bijective_of_not_crit {n k kk : ℕ}
    ...
    (hexp : 2 ^ k + 1 < 2 ^ n - 1) :
    ¬ Function.Bijective (Thm8C1.gmap (F := F) n k kk) := by
  intro hbij; have := hbij.injective; simp_all +decide [ Thm8C1.gmap ] ;
  convert @this 0 1 ?_ using 1;
  · simp +decide;
  · unfold gmap; simp +decide [ Dobbertin.Thm5.qeps, DempwolffMueller.truncTrace ] ;
...
theorem gmap_bijective_iff {n k kk : ℕ} (hn : Fintype.card F = 2 ^ n)
    (hk : 0 < k) (hkn : k < n) (hcop : Nat.Coprime k n) (hkk' : k * kk % n = 1)
    (hexp : 2 ^ k + 1 < 2 ^ n - 1) :
    Function.Bijective (Thm8C1.gmap (F := F) n k kk) ↔ Odd (kk + n) := by
  constructor
  · intro hbij
    by_contra hcrit
