...
/-!
### F as an abstract endomorphism

The Frobenius is not just a set map: additivity (`frob_add`) makes each
`frob step` an *endomorphism of the additive group* `F`, i.e. an element
`frobEndo step` of the endomorphism ring `AddMonoid.End F`.  This is the bridge
that lets `DobbertinLego/Endo`'s generic scaffold see `frob`:

* composition of iterates (`frob_comp`) becomes the ring power
  `frobEndo step ^ j = frobEndo (j·step)` (`frobEndo_pow_apply`);
* Fermat (`frob_card`) becomes finite order `frobEndo n = 1`
  (`frobEndo_pow_card`).
-/

/-- **Gadget F as an endomorphism.**  `frob step` packaged as an element of the
endomorphism ring `AddMonoid.End F`; additivity `frob_add` is what makes this an
additive-group homomorphism. -/
def frobEndo (step : ℕ) : AddMonoid.End F :=
  AddMonoidHom.mk' (frob step) (frob_add step)

omit [Fintype F] in
@[simp] lemma frobEndo_apply (step : ℕ) (x : F) : frobEndo step x = frob step x := rfl

omit [Fintype F] in
/-- **Iterates are ring powers**: the `j`-th power of `frobEndo step` in the
endomorphism ring is `frob (j·step)` — composition (`frob_comp`) read as
multiplication in `AddMonoid.End F`. -/
lemma frobEndo_pow_apply (step j : ℕ) (x : F) :
    (frobEndo (F := F) step ^ j) x = frob (j * step) x := by
  induction j with
  | zero => simp [frob]
  | succ m ih =>
    rw [pow_succ']
    show frobEndo step ((frobEndo step ^ m) x) = _
    rw [ih]; simp only [frobEndo_apply, frob_comp]; ring_nf

omit [Fintype F] in
/-- **Powers collapse**: `frobEndo step ^ j = frobEndo (j·step)` in the endomorphism
ring — the `AddMonoid.End` form of `frob_comp`. -/
lemma frobEndo_pow (step j : ℕ) : frobEndo (F := F) step ^ j = frobEndo (j * step) := by
  refine DFunLike.ext _ _ (fun x => ?_)
  rw [frobEndo_pow_apply, frobEndo_apply]

/-- **Finite order**: on `𝔽_{2ⁿ}` the full-length Frobenius endomorphism is the
identity of the endomorphism ring, `frobEndo n = 1` — the endomorphism-ring form
of Fermat (`frob_card`). -/
lemma frobEndo_pow_card {n : ℕ} (hn : Fintype.card F = 2 ^ n) :
    frobEndo (F := F) n = 1 := by
  refine DFunLike.ext _ _ (fun x => ?_)
  rw [frobEndo_apply, frob_card hn]; rfl
