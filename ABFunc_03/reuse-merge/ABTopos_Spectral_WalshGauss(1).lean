...
lemma χ_add (x y : 𝔽) : χ_ 𝔽 (x + y) = χ_ 𝔽 x * χ_ 𝔽 y := by
  convert congr_arg _ ( map_add ( AbsTrace 𝔽 ) x y ) using 1;
  any_goals exact fun x => ( -1 : ℂ ) ^ x.val;
  · rfl;
  · unfold χ_;
    cases' Fin.exists_fin_two.mp ⟨ ( AbsTrace 𝔽 x ), rfl ⟩ with hx hx <;> cases' Fin.exists_fin_two.mp ⟨ ( AbsTrace 𝔽 y ), rfl ⟩ with hy hy <;> simp +decide [ hx, hy, pow_add ]
...
theorem gauss_norm (ψ : 𝔽ˣ →* ℂˣ) (hψ : ψ ≠ 1) :
    ‖𝔤 𝔽 ψ‖ = Real.sqrt (Fintype.card 𝔽 : ℝ) := by
      rw [ ← sq_eq_sq₀ ];
      · rw [ Real.sq_sqrt ( Nat.cast_nonneg _ ), stickelberger_norm 𝔽 ψ hψ ];
      · positivity;
      · positivity
...
theorem walsh_parseval (d : ℕ) :
    ∑ u : 𝔽, ‖Ŵ 𝔽 d u‖ ^ 2 = (Fintype.card 𝔽 : ℝ) ^ 2 := by
      have h_eval : ∀ u, ‖Ŵ 𝔽 d u‖ ^ 2 = ∑ x : 𝔽, ∑ y : 𝔽, χ_ 𝔽 (u * x + x ^ d) * χ_ 𝔽 (u * y + y ^ d) := by
        intro u
        have h_eval : ‖Ŵ 𝔽 d u‖ ^ 2 = (∑ x : 𝔽, χ_ 𝔽 (u * x + x ^ d)) * (∑ y : 𝔽, χ_ 𝔽 (u * y + y ^ d)) := by
          unfold Ŵ;
          norm_num [ χ_ ];
          norm_cast ; simp +decide [ ← sq ];
        rw [ h_eval, Finset.sum_mul ];
        simp +decide only [Finset.mul_sum _ _ _];
      -- By Fubini's theorem, we can interchange the order of summation.
      have h_fubini : ∑ u : 𝔽, ∑ x : 𝔽, ∑ y : 𝔽, χ_ 𝔽 (u * x + x ^ d) * χ_ 𝔽 (u * y + y ^ d) = ∑ x : 𝔽, ∑ y : 𝔽, ∑ u : 𝔽, χ_ 𝔽 (u * (x - y)) * χ_ 𝔽 (x ^ d + y ^ d) := by
        have h_fubini : ∀ u x y : 𝔽, χ_ 𝔽 (u * x + x ^ d) * χ_ 𝔽 (u * y + y ^ d) = χ_ 𝔽 (u * (x - y)) * χ_ 𝔽 (x ^ d + y ^ d) := by
          intro u x y
          have h_char : χ_ 𝔽 (u * x + x ^ d) * χ_ 𝔽 (u * y + y ^ d) = χ_ 𝔽 ((u * x + x ^ d) + (u * y + y ^ d)) := by
            convert χ_add 𝔽 ( u * x + x ^ d ) ( u * y + y ^ d ) |> Eq.symm using 1;
          have h_char : χ_ 𝔽 ((u * x + x ^ d) + (u * y + y ^ d)) = χ_ 𝔽 (u * (x - y)) * χ_ 𝔽 (x ^ d + y ^ d) := by
            have h_char : (u * x + x ^ d) + (u * y + y ^ d) = u * (x - y) + (x ^ d + y ^ d) := by
              grind
            rw [ h_char, χ_add ];
          aesop;
        exact Finset.sum_comm.trans ( Finset.sum_congr rfl fun _ _ => Finset.sum_comm.trans ( Finset.sum_congr rfl fun _ _ => Finset.sum_congr rfl fun _ _ => h_fubini _ _ _ ) );
      -- By orthogonality, we know that $\sum_{u} \chi(u(x-y)) = |𝔽|$ if $x = y$ and $0$ otherwise.
      have h_orthogonality : ∀ x y : 𝔽, ∑ u : 𝔽, χ_ 𝔽 (u * (x - y)) = if x = y then (Fintype.card 𝔽 : ℂ) else 0 := by
        intro x y; split_ifs with h; simp +decide [ h ] ;
        · unfold χ_; simp +decide ;
        · have := χ_orthogonality 𝔽 ( x - y );
          simpa only [ mul_comm ] using this.trans ( if_neg ( sub_ne_zero_of_ne h ) );
      simp_all +decide [ ← Finset.mul_sum _ _ _, ← Finset.sum_mul ];
      convert congr_arg Complex.re h_fubini using 1;
      · rw [ ← Finset.sum_congr rfl fun _ _ => h_eval _ ] ; norm_cast;
      · simp +decide [ ← two_mul, χ_sq ];
        simp +decide [ show ( 2 : 𝔽 ) = 0 by exact CharP.cast_eq_zero 𝔽 2 ];
        simp +decide [ sq, χ_ ]
