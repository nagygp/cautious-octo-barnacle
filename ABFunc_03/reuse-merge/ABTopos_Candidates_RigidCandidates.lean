...
def kerdockLift (k : ℕ) [NeZero k] (f : Multiplicative (ZMod k) → Multiplicative (ZMod k))
    (hf : f 1 = 1) : ABFunc TypeTopos :=
  mkABFunc (Multiplicative (ZMod k)) f hf
...
def dualityFixedPointLift (G : Type) [Group G] (aut : G → G) (hf : aut 1 = 1) :
    ABFunc TypeTopos :=
  mkABFunc G aut hf
...
def eulerPatternLift (G : Type) [Group G] (f : G → G) (hf : f 1 = 1) :
    ABFunc TypeTopos :=
  mkABFunc G f hf
...
def AB1 : ABFunc TypeTopos := kerdockLift 8 (fun x => x ^ 3) (one_pow _)

/-- **AB2**: Kasami exponent on Z/32Z — models GF(2⁵) power map x^5. -/
def AB2 : ABFunc TypeTopos := kerdockLift 32 (fun x => x ^ 5) (one_pow _)
...
def AB3 : ABFunc TypeTopos := kerdockLift 128 (fun x => x ^ 9) (one_pow _)

/-- **AB4**: Welch exponent on Z/32Z — models GF(2⁵) power map x^7.
    The Welch exponent d = 2^t + 3 where 2t + 1 = n. -/
def AB4 : ABFunc TypeTopos := kerdockLift 32 (fun x => x ^ 7) (one_pow _)

/-- **AB5**: Niho exponent on Z/512Z — models GF(2⁹) power map x^{2⁴+2²−1} = x^19. -/
def AB5 : ABFunc TypeTopos := kerdockLift 512 (fun x => x ^ 19) (one_pow _)
...
def AB6 : ABFunc TypeTopos :=
  dualityFixedPointLift (Equiv.Perm (Fin 4)) (fun σ => σ⁻¹) inv_one
...
def AB7 : ABFunc TypeTopos :=
  dualityFixedPointLift (Multiplicative (ZMod 16)) (fun x => x⁻¹) inv_one
...
def AB9 : ABFunc TypeTopos :=
  eulerPatternLift (Multiplicative (ZMod 64)) (fun x => x ^ 2) (one_pow _)
...
def AB10 : ABFunc TypeTopos :=
  eulerPatternLift (Multiplicative (ZMod 256)) (fun x => x ^ 3) (one_pow _)
...
structure APNCandidateData where
  ...
  /-- Proof that the candidate preserves the group identity -/
  func_one : func 1 = 1
...
def APNCandidateData.toABFunc (c : APNCandidateData) : ABFunc TypeTopos :=
  mkABFunc (Multiplicative (ZMod c.groupSize)) c.func c.func_one
...
def orbitMixedABFunc (m k₁ k₂ : ℕ) [NeZero m] : ABFunc TypeTopos :=
  mkABFunc (Multiplicative (ZMod m)) (orbitMixedFunc m k₁ k₂) (one_pow _)
