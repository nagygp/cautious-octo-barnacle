...
/-- Absolute trace  Tr : GF(2ⁿ) → GF(2), defined via the algebra trace
    `Algebra.trace (ZMod 2) 𝔽`. -/
def AbsTrace : 𝔽 →+ ZMod 2 := by
  letI : Algebra (ZMod 2) 𝔽 := ZMod.algebra _ _
  exact (Algebra.trace (ZMod 2) 𝔽).toAddMonoidHom

/-- The absolute trace is surjective (finite fields are separable). -/
lemma AbsTrace_surjective : Function.Surjective (AbsTrace 𝔽) := by
  letI : Algebra (ZMod 2) 𝔽 := ZMod.algebra _ _
  haveI : FiniteDimensional (ZMod 2) 𝔽 := inferInstance
  haveI : Algebra.IsSeparable (ZMod 2) 𝔽 := inferInstance
  exact Algebra.trace_surjective (ZMod 2) 𝔽
...
lemma χ_add (x y : 𝔽) : χ_ 𝔽 (x + y) = χ_ 𝔽 x * χ_ 𝔽 y := by
  unfold χ_;
  cases' Fin.exists_fin_two.1 ⟨ ( AbsTrace 𝔽 ) x, rfl ⟩ with hx hx <;> cases' Fin.exists_fin_two.1 ⟨ ( AbsTrace 𝔽 ) y, rfl ⟩ with hy hy <;> simp +decide [ hx, hy ]
...
/-- χ at zero is 1. -/
lemma χ_zero : χ_ 𝔽 0 = 1 := by
  simp [χ_, map_zero]

/-- χ values are units (either 1 or -1). -/
lemma χ_ne_zero (x : 𝔽) : χ_ 𝔽 x ≠ 0 := by
  intro h
  have := χ_sq 𝔽 x
  rw [h, zero_pow (by norm_num : 2 ≠ 0)] at this
  exact zero_ne_one this

/-- For a ≠ 0, multiplication by a is a bijection on 𝔽. -/
lemma mul_left_bijective (a : 𝔽) (ha : a ≠ 0) : Function.Bijective (fun x : 𝔽 => a * x) :=
  ⟨mul_right_injective₀ ha, mul_left_surjective₀ ha⟩

/-- Helper: summing χ over 𝔽 for a ≠ 0 gives 0, using the shift trick.
    The idea: let S = ∑_x χ(ax). Pick y such that χ(ay) ≠ 1.
    Then S = ∑_x χ(a(x+y)) = χ(ay) · S by reindexing and χ_add.
    So (1 - χ(ay)) · S = 0, hence S = 0. -/
lemma χ_sum_eq_zero (a : 𝔽) (ha : a ≠ 0) :
    ∑ x : 𝔽, χ_ 𝔽 (a * x) = 0 := by
  sorry
...
lemma χ_orthogonality (a : 𝔽) :
  ...
  split
  · subst_vars; simp [χ_zero]
  · exact χ_sum_eq_zero 𝔽 a ‹_›
...
theorem gauss_norm (ψ : 𝔽ˣ →* ℂˣ) (hψ : ψ ≠ 1) :
    ‖𝔤 𝔽 ψ‖ = Real.sqrt (Fintype.card 𝔽 : ℝ) := by
      convert congr_arg Real.sqrt ( stickelberger_norm 𝔽 ψ hψ ) using 1 ; norm_num
...
theorem walsh_parseval (d : ℕ) :
    ∑ u : 𝔽, ‖Ŵ 𝔽 d u‖ ^ 2 = (Fintype.card 𝔽 : ℝ) ^ 2 := by
      ...
      have h_fubini : ∑ u : 𝔽, ‖∑ x : 𝔽, χ_ 𝔽 (u * x + x ^ d)‖ ^ 2 = ∑ x : 𝔽, ∑ y : 𝔽, ∑ u : 𝔽, χ_ 𝔽 (u * x + x ^ d - (u * y + y ^ d)) := by
        have h_sum : ∀ u : 𝔽, ‖∑ x : 𝔽, χ_ 𝔽 (u * x + x ^ d)‖ ^ 2 = ∑ x : 𝔽, ∑ y : 𝔽, χ_ 𝔽 (u * x + x ^ d) * χ_ 𝔽 (u * y + y ^ d) := by
          intro u
          have h_sum : ‖∑ x : 𝔽, χ_ 𝔽 (u * x + x ^ d)‖ ^ 2 = (∑ x : 𝔽, χ_ 𝔽 (u * x + x ^ d)) * (∑ y : 𝔽, χ_ 𝔽 (u * y + y ^ d)) := by
            unfold χ_;
            norm_cast ; simp +decide [ sq ];
          rw [ h_sum, Finset.sum_mul ];
          simp +decide only [Finset.mul_sum _ _ _];
        push_cast [ ← Finset.sum_product', h_sum ];
        refine' Finset.sum_bij ( fun x _ => ( x.2.2, x.2.1, x.1 ) ) _ _ _ _ <;> simp +decide;
        intro a b c; rw [ ← χ_add ] ; ring;
        rw [ show a * b + a * c + b ^ d + c ^ d = - ( a * b ) + ( a * c - b ^ d ) + c ^ d + ( a * b + a * b + b ^ d + b ^ d ) by ring ] ; simp +decide [ ← two_mul, CharTwo.two_eq_zero ] ;
      -- By orthogonality, $\sum_{u} \chi(u(x-y)) = |𝔽|$ if $x = y$ and $0$ otherwise.
      have h_orthogonality : ∀ x y : 𝔽, ∑ u : 𝔽, χ_ 𝔽 (u * (x - y)) = if x = y then (Fintype.card 𝔽 : ℂ) else 0 := by
        intro x y; split_ifs with hxy <;> simp_all +decide [ mul_comm ] ;
        · simp +decide [ χ_ ];
        · have := χ_orthogonality 𝔽 ( x - y ) ; simp_all +decide [ sub_eq_iff_eq_add ] ;
          simpa only [ mul_comm ] using this;
      -- Apply the orthogonality result to simplify the inner sum.
      have h_inner_sum : ∀ x y : 𝔽, ∑ u : 𝔽, χ_ 𝔽 (u * x + x ^ d - (u * y + y ^ d)) = if x = y then (Fintype.card 𝔽 : ℂ) else 0 := by
        intro x y; specialize h_orthogonality x y; simp_all +decide [ mul_sub, sub_add_eq_sub_sub ] ;
        convert congr_arg ( fun z => z * χ_ 𝔽 ( x ^ d - y ^ d ) ) h_orthogonality using 1 <;> ring;
        · rw [ Finset.sum_mul _ _ _ ] ; congr ; ext ; rw [ ← mul_comm ] ; rw [ ← χ_add ] ; ring;
        · aesop;
      simp_all +decide [ sq ];
      exact_mod_cast h_fubini
