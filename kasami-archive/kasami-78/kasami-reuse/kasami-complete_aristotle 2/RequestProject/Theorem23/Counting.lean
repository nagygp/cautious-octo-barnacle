...
/-- The Walsh coefficient: `W_f(a, b) = ∑_{x ∈ F} ψ(a·x + b·f(x))`. -/
noncomputable def WalshCoeff (ψ : AddChar F ℂ) (f : F → F) (a b : F) : ℂ :=
  ∑ x : F, ψ (a * x + b * f x)

/-- APN (Almost Perfect Nonlinear): `f(x + u) + f(x) = v` has at most 2 solutions
    for every nonzero `u`. -/
...
/-! ### Task 1: The Walsh-Differential Identity

Ref: Budaghyan Theorem 2.3, Equations (2.1)–(2.5).

The fourth power moment form:
...
/-- If k is even, then k² ≥ 2k. Used in the char 2 pairing argument (Step C). -/
lemma sq_ge_two_mul_of_even (k : ℕ) (hk : 2 ∣ k) : k ^ 2 ≥ 2 * k := by
  cases k <;> simp_all +decide;
  nlinarith [Nat.le_of_dvd (Nat.succ_pos _) hk]

/-- If k² ≤ 2k for natural k, then k ≤ 2. Used in the forcing argument (Step D). -/
...
/-- For AB functions, W(a,b)⁴ = W(a,b)² · 2^{n+1} when b ≠ 0 (Step A). -/
...
/-- Pointwise lower bound: for each u ≠ 0, ∑_v δ(u,v)² ≥ 2 · ∑_v δ(u,v).
    Since δ(u,v) is even for u ≠ 0, we have δ(u,v)² ≥ 2·δ(u,v) pointwise. -/
lemma sum_sq_ge_two_mul_sum_of_even
  ...
  exact Finset.sum_le_sum fun v _ => by
    nlinarith only [sq_ge_two_mul_of_even (δ u v) (H_even v)]
...
theorem AB_implies_APN
    ...
    IsAPN_abs δ := by
  -- Step 1: Compute ∑ δ² from the fourth moment identity.
    ...
    have h_sum_eq : ∑ u ∈ Finset.univ, ∑ b ∈ Finset.univ, W u b ^ 4 = (q : ℤ) ^ 4 + 2 ^ (n + 1) * ((Fintype.card ι : ℤ) - 1) * (q : ℤ) ^ 2 := by
      have h_sum_eq : ∑ u ∈ Finset.univ, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W u b ^ 4 = 2 ^ (n + 1) * ((Fintype.card ι : ℤ) - 1) * (q : ℤ) ^ 2 := by
        have h_sum_eq : ∑ u ∈ Finset.univ, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W u b ^ 4 = ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), ∑ u ∈ Finset.univ, W u b ^ 2 * 2 ^ (n + 1) := by
          rw [Finset.sum_comm];
          exact Finset.sum_congr rfl fun b hb => Finset.sum_congr rfl fun a _ha => AB_fourth_eq_second_times_pow W n hAB a b (Finset.mem_filter.mp hb |>.2) ▸ rfl
  ...
  -- Step 2: Show ∑_v δ(u,v)² = 2·∑_v δ(u,v) for each nonzero u.
  have h_eq : ∀ u ∈ Finset.univ.erase 0, ∑ v ∈ Finset.univ, (δ u v : ℤ) ^ 2 = 2 * ∑ v ∈ Finset.univ, (δ u v : ℤ) := by
    have h_eq : ∑ u ∈ Finset.univ.erase 0, ∑ v ∈ Finset.univ, (δ u v : ℤ) ^ 2 = ∑ u ∈ Finset.univ.erase 0, 2 * (∑ v ∈ Finset.univ, (δ u v : ℤ)) := by
      ...
      rw [h_eq, Finset.sum_congr rfl fun u hu => by rw [H_row_sum u (Finset.ne_of_mem_erase hu)]]; simp +decide [hcard]; ring_nf;
      rw [Nat.cast_sub] <;> push_cast <;> nlinarith only [hq, Nat.pow_le_pow_right two_pos hn]
    -- If some row had strict inequality, the total would be strictly larger.
    contrapose! h_eq;
    ...
    · obtain ⟨u, hu, hu'⟩ := h_eq;
      exact ⟨u, hu, lt_of_le_of_ne (sum_sq_ge_two_mul_sum_of_even δ u (Finset.ne_of_mem_erase hu) (fun v => H_even u (Finset.ne_of_mem_erase hu) v)) hu'.symm⟩
  -- Step 3: Deduce δ(u,v) ≤ 2 for each nonzero u and all v.
  intro u hu v
...
omit [DecidableEq ι] in
/-- Number of unordered pairs in the Walsh support equals 2^{n-2} · (2^{n-1} - 1). -/
