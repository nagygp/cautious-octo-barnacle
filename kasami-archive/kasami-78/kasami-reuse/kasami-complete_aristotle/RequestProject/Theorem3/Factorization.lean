...
/-- The Frobenius endomorphism `φ(x) = x^2` in characteristic 2. -/
...
lemma card_ker_L₂ (hk : 0 < k) :
  ...
  -- The polynomial p := ∑_{i=0}^{k-1} X^{2^i} has degree 2^{k-1}.
  set p : Polynomial F := ∑ i ∈ Finset.range k, Polynomial.X ^ (2 ^ i)
  -- Roots of L₂ are roots of p.
  have h_roots :
      Finset.filter (fun y : F => L₂ k F y = 0) Finset.univ ⊆ p.roots.toFinset := by
    intro y hy
    simp_all +decide
    refine ⟨?_, ?_⟩
    · simp +zetaDelta at *
      exact ne_of_apply_ne (fun p => p.coeff (2 ^ (k - 1)))
        (by cases k <;> simp_all +decide [Polynomial.coeff_eq_zero_of_natDegree_lt])
    · rw [Polynomial.eval_finset_sum,
        show L₂ k F y = ∑ i ∈ Finset.range k, y ^ (2 ^ i) from rfl] at *
      aesop
  refine le_trans (Finset.card_le_card h_roots)
    (le_trans (Multiset.toFinset_card_le _)
      (le_trans (Polynomial.card_roots' _) ?_))
...
/-- **Root count for `L₀`:** `|{y ∈ F : y^(2^k) + y = 0}| ≤ 2^k`.
    The polynomial `X^(2^k) + X` has degree `2^k`, so at most `2^k` roots. -/
lemma card_roots_L₀_le (hk : 0 < k) :
  ...
  have h_roots_bound : (p.roots.toFinset).card ≤ 2 ^ k := by
    refine le_trans (Multiset.toFinset_card_le _)
      (le_trans (Polynomial.card_roots' _) ?_)
  ...
  exact ⟨ne_of_apply_ne Polynomial.natDegree (by
    ...
    linarith [Nat.pow_le_pow_right two_pos hk]), by aesop⟩
...
lemma card_roots_shifted_le (hk : 0 < k) :
  ...
  have h_roots :
      (Finset.filter (fun y : F => y ^ 2 ^ k + y + 1 = 0) Finset.univ).card ≤
        (Polynomial.X ^ 2 ^ k + Polynomial.X + 1 : Polynomial F).roots.toFinset.card := by
    refine Finset.card_le_card ?_
    simp +decide [Finset.subset_iff]
    exact fun x hx =>
      ⟨by exact ne_of_apply_ne (Polynomial.eval 0) (by simp +decide), hx⟩
  refine le_trans h_roots
    (le_trans (Multiset.toFinset_card_le _)
      (le_trans (Polynomial.card_roots' _) ?_))
