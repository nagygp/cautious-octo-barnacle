...
theorem AB_implies_APN
    ...
    IsAPN_abs δ := by
  -- By the equality part of the fourth moment identity, we have that $\sum_{u \neq 0} \sum_v \delta(u, v)^2 = 2q(q - 1)$.
  have h_eq : ∑ u ∈ Finset.univ.filter (· ≠ (0 : ι)), ∑ v : ι, (δ u v : ℤ) ^ 2 = 2 * q * (q - 1) := by
    have h_eq : ∑ u : ι, ∑ v : ι, (δ u v : ℤ) ^ 2 = 3 * q ^ 2 - 2 * q := by
      -- We need to show that $\sum_{a,b} W(a,b)^4 = q^4 + 2q(q-1)q^2$.
      have h_sum_fourth : ∑ a, ∑ b, W a b ^ 4 = q ^ 4 + 2 * q * (q - 1) * q ^ 2 := by
        have h_fourth_moment_split : ∑ a : ι, ∑ b : ι, W a b ^ 4 = ∑ a : ι, W a (0 : ι) ^ 4 + ∑ a : ι, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W a b ^ 4 := by
          simp +decide [ Finset.sum_ite, Finset.filter_ne' ]
        generalize_proofs at *;
        have h_fourth_moment_split : ∑ a : ι, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W a b ^ 4 = ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), ∑ a : ι, W a b ^ 2 * (2 : ℤ) ^ (n + 1) := by
          rw [ Finset.sum_comm ];
          refine' Finset.sum_congr rfl fun b hb => Finset.sum_congr rfl fun a ha => _;
          cases hAB a b ( by simpa using hb ) <;> simp_all +decide [ pow_succ, mul_assoc ]
        generalize_proofs at *;
        simp_all +decide [ ← Finset.sum_mul _ _ _ ];
        rw [ ← H_fourth_moment ] ; ring;
        rw [ Finset.sum_eq_single 0 ] <;> simp_all +decide [ Finset.filter_ne' ] ; ring;
      exact mul_left_cancel₀ ( pow_ne_zero 2 ( by linarith [ Nat.pow_le_pow_right two_pos hn ] : ( q : ℤ ) ≠ 0 ) ) ( by linarith );
    simp_all +decide [ Finset.filter_ne' ];
    rw [ Finset.sum_eq_single 0 ] <;> simp_all +decide [ sq ] ; ring;
  -- For each $u \neq 0$, we have $\sum_v \delta(u, v)^2 = 2 \sum_v \delta(u, v)$.
  have h_eq_u : ∀ u, u ≠ 0 → ∑ v : ι, (δ u v : ℤ) ^ 2 = 2 * ∑ v : ι, (δ u v : ℤ) := by
    have h_eq_u : ∑ u ∈ Finset.univ.filter (· ≠ (0 : ι)), ∑ v : ι, (δ u v : ℤ) ^ 2 = ∑ u ∈ Finset.univ.filter (· ≠ (0 : ι)), 2 * ∑ v : ι, (δ u v : ℤ) := by
      simp_all +decide [ Finset.sum_ite, Finset.filter_ne' ];
      rw [ ← Finset.mul_sum _ _ _, Finset.sum_eq_add_sum_diff_singleton ( Finset.mem_univ 0 ) ];
      rw [ Finset.sum_congr rfl fun x hx => H_row_sum x ( by aesop ) ] ; simp +decide [ Finset.card_sdiff, * ] ; ring;
    contrapose! h_eq_u;
    refine' ne_of_gt ( Finset.sum_lt_sum _ _ );
    · exact fun u hu => sum_sq_ge_two_mul_sum_of_even δ u ( Finset.mem_filter.mp hu |>.2 ) ( H_even u ( Finset.mem_filter.mp hu |>.2 ) );
    · obtain ⟨ u, hu, hu' ⟩ := h_eq_u; use u; simp_all +decide ;
      have h_eq_u : ∑ v : ι, (δ u v : ℤ) ^ 2 ≥ 2 * ∑ v : ι, (δ u v : ℤ) := by
        exact?;
      exact lt_of_le_of_ne ( by linarith [ H_row_sum u hu ] ) ( Ne.symm hu' );
  -- For each $u \neq 0$, we have $\delta(u, v)^2 = 2 \delta(u, v)$ for all $v$.
  have h_eq_uv : ∀ u, u ≠ 0 → ∀ v, (δ u v : ℤ) ^ 2 = 2 * (δ u v : ℤ) := by
    intros u hu v
    have h_eq_uv_step : ∀ v, (δ u v : ℤ) ^ 2 ≥ 2 * (δ u v : ℤ) := by
      intro v
      by_cases hv : δ u v = 0;
      · simp [hv];
      · nlinarith only [ show ( δ u v : ℤ ) ≥ 2 by exact_mod_cast Nat.le_of_dvd ( Nat.pos_of_ne_zero hv ) ( H_even u hu v ) ];
    contrapose! h_eq_u;
    exact ⟨ u, hu, ne_of_gt <| by simpa only [ Finset.mul_sum _ _ _ ] using Finset.sum_lt_sum ( fun v _ => h_eq_uv_step v ) ⟨ v, Finset.mem_univ v, lt_of_le_of_ne ( h_eq_uv_step v ) h_eq_u.symm ⟩ ⟩;
  exact fun u hu v => by nlinarith only [ h_eq_uv u hu v ] ;
...
theorem triple_count_eq
  ...
  have h_sum : ∑ a, W a b ^ 2 = ∑ a ∈ walshSupport W b, 2 ^ (n + 1) := by
    rw [ ← Finset.sum_subset ( Finset.subset_univ ( walshSupport W b ) ) ];
    · exact Finset.sum_congr rfl fun x hx => Or.resolve_left ( hAB x b hb ) fun h => Finset.mem_filter.mp hx |>.2 <| by simp_all +decide ;
