...
theorem AB_implies_APN
    ...
    IsAPN_abs δ := by
  -- From H_fourth_moment, we have q^2 * sum_{u,v} δ(u,v)^2 = q^4 + 2^(n+1) * (q-1) * q^2.
  have h_sum_sq : ∑ u, ∑ v, (δ u v : ℤ) ^ 2 = 3 * q ^ 2 - 2 * q := by
    have h_sum_sq : ∑ a, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W a b ^ 4 = 2 ^ (n + 1) * (q : ℤ) ^ 2 * ((Fintype.card ι : ℤ) - 1) := by
      have h_sum_sq : ∑ a, ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), W a b ^ 4 = ∑ b ∈ Finset.univ.filter (· ≠ (0 : ι)), ∑ a, W a b ^ 2 * (2 : ℤ) ^ (n + 1) := by
        rw [ Finset.sum_comm, Finset.sum_congr rfl ];
        exact fun x hx => Finset.sum_congr rfl fun y hy => AB_fourth_eq_second_times_pow W n hAB y x ( by simpa using hx );
    ...
    rw [ show ( ∑ x : ι, W x 0 ^ 4 : ℤ ) = ( 2 ^ n ) ^ 4 by
          rw [ Finset.sum_eq_single 0 ] <;> simp_all +decide [ pow_succ' ] ] at h_sum_sq;
    exact eq_sub_of_add_eq <| mul_left_cancel₀ ( pow_ne_zero 2 ( pow_ne_zero n two_ne_zero ) ) <| by linear_combination h_sum_sq;
  -- From H_even and H_row_sum, we have sum_{u≠0} sum_{v} δ(u,v)² = 2 * sum_{u≠0} sum_{v} δ(u,v).
  have h_sum_sq_eq : ∑ u ∈ Finset.univ.filter (· ≠ 0), ∑ v, (δ u v : ℤ) ^ 2 = 2 * ∑ u ∈ Finset.univ.filter (· ≠ 0), ∑ v, (δ u v : ℤ) := by
    have h_eq_sq : ∑ u ∈ Finset.univ.filter (· ≠ 0), ∑ v, (δ u v : ℤ) ^ 2 = 3 * q ^ 2 - 2 * q - q ^ 2 := by
      simp_all +decide [ Finset.filter_ne' ];
      rw [ Finset.sum_eq_single 0 ] <;> aesop;
    rw [ h_eq_sq, Finset.sum_congr rfl fun u hu => H_row_sum u <| Finset.mem_filter.mp hu |>.2 ] ; simp +decide [ Finset.filter_ne' ] ; ring;
    rw [ Nat.cast_sub ] <;> push_cast [ hcard ] <;> nlinarith only [ hq, hn, Nat.pow_le_pow_right two_pos hn ];
  -- From H_even and H_row_sum, we have δ(u,v)² = 2 * δ(u,v) for all u ≠ 0 and v.
  have h_delta_sq_eq_two_delta : ∀ u ≠ 0, ∀ v, (δ u v : ℤ) ^ 2 = 2 * (δ u v : ℤ) := by
    have h_delta_sq_eq_two_delta : ∀ u ≠ 0, ∀ v, (δ u v : ℤ) ^ 2 ≥ 2 * (δ u v : ℤ) := by
      exact fun u hu v => by nlinarith only [ sq_ge_two_mul_of_even ( δ u v ) ( H_even u hu v ) ] ;
    contrapose! h_sum_sq_eq;
    rw [ Finset.mul_sum _ _ _ ];
    refine' ne_of_gt ( Finset.sum_lt_sum _ _ );
    · exact fun u hu => by rw [ Finset.mul_sum _ _ _ ] ; exact Finset.sum_le_sum fun v _ => h_delta_sq_eq_two_delta u ( Finset.mem_filter.mp hu |>.2 ) v;
    · obtain ⟨ u, hu, v, hv ⟩ := h_sum_sq_eq;
      exact ⟨ u, by simpa using hu, by rw [ Finset.mul_sum _ _ _ ] ; exact Finset.sum_lt_sum ( fun v _ => h_delta_sq_eq_two_delta u hu v ) ⟨ v, Finset.mem_univ v, lt_of_le_of_ne ( h_delta_sq_eq_two_delta u hu v ) hv.symm ⟩ ⟩;
  intro u hu v;
  nlinarith only [ h_delta_sq_eq_two_delta u hu v ]
...
theorem triple_count_eq
  ...
  -- From the AB property, we have W(a, b)^2 = 2^(n+1) for every a ∈ walshSupport W b.
  have hWalshSquare : ∑ a ∈ walshSupport W b, W a b ^ 2 = 2 ^ (n + 1) * (walshSupport W b).card := by
    have hWalshSquare : ∀ a ∈ walshSupport W b, W a b ^ 2 = 2 ^ (n + 1) := by
      exact fun a ha => Or.resolve_left ( hAB a b hb ) ( by simpa using Finset.mem_filter.mp ha |>.2 );
    simpa [ mul_comm ] using Finset.sum_congr rfl hWalshSquare;
  -- From the Parseval equality, we have ∑ a, W a b ^ 2 = q ^ 2 = (2 ^ n) ^ 2 = 2 ^ (2 * n).
  have hParseval : ∑ a ∈ walshSupport W b, W a b ^ 2 = 2 ^ (2 * n) := by
    convert H_parseval b using 1;
    · exact Finset.sum_subset ( Finset.filter_subset _ _ ) fun x hx₁ hx₂ => by simp_all +decide [ walshSupport ] ;
    · norm_num [ hq, pow_mul' ];
...
theorem triple_count_pairs
    ...
    Nat.choose (walshSupport W b).card 2 =
      2 ^ (n - 2) * (2 ^ (n - 1) - 1) := by
  have h_card : (walshSupport W b).card = 2 ^ (n - 1) := by
