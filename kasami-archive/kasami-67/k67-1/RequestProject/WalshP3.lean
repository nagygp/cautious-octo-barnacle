...
**Note:** The `p3_triple_count` theorem at the end of this file proves
T₃ = 2^{2n-3} from hypotheses `hTriple` and `hTripleSum`. However, these
hypotheses are mathematically incorrect (see `KasamiFinal.lean` for the
corrected version T₃ = 2^{2n-3} - 2^{n-2}, verified by computation over GF(2³)).
