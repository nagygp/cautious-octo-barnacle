...
/-- Under the hypotheses and a ≠ 0, the kernel of L_a has at most 2 elements
    (since kerL ⊆ {0,1} by Phase 1), so `kerDimLA k a ∈ {1, 2}`.
    The value 1 occurs when 0 is the only root; 2 when both 0 and 1 are roots.

    Note: a ≠ 0 is essential; when a = 0, L_a = 0 and kerDimLA = |F| = 2^n. -/
...
Note: a ≠ 0 is required. When a = 0, the function reduces to the identity
and the WHT squared is (2^n)² at b = 0, which is not in {0, 2^n, 2^{n+1}}. -/
...
theorem kasami_wht_sq_value {n k : ℕ} (hn : n ≠ 0) (hk : k ≠ 0)
  ...
  · exact Or.inl (by rw [show whtSqMag k a b = Complex.normSq (wht k a b) from rfl,
      wht_vanishing k a b h, Complex.normSq_zero])
  · have hpeak := wht_peak k n a b hn hcard
