...
/-! ### Twisted cube inclusion (leaf lemma)

For elements in ker(L_a) with a ≠ 0, we need to show they satisfy
the Frobenius fixed-point condition z^{2^{3k}} = z.
This is proved by applying the Frobenius to the equation L_a(z) = 0
and algebraic manipulation in characteristic 2. -/

/-- Elements of ker(L_a) satisfy the cube Frobenius condition.
    This is the twisted analogue of `linearized_kernel_subset_cube`. -/
lemma linPolyLA_ker_subset_cube {k : ℕ} {a z : F} (ha : a ≠ 0)
    (hz : linPolyLA k a z = 0) :
    z ^ (2 ^ (3 * k)) = z := by
  sorry

/-! ### Kernel dimension bound -/

/-- Zero is always in the kernel of L_a. -/
...
lemma kerDimLA_pos (k : ℕ) (a : F) : 1 ≤ kerDimLA (F := F) k a := by
  ...
  exact Set.Nonempty.one_le_ncard hne (Set.toFinite _)
